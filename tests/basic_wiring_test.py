...
class TestMainFunctions():
    ...
    async def test_volume(self):
        ...
        await self._test_sending_commands(SETUP_RESPONSES, "AUDTYPE", client_functions, test_function ) 
    ...
    async def _test_receiving_commands(self, commands_received, wait_for_command, test_function):
        transport = mock.Mock()
    ...
    async def _test_sending_commands(self, commands_received, wait_for_command, client_functions, test_function):
        transport = mock.Mock()
        ...
        with mock.patch("asyncio.get_event_loop", new_callable=mock.Mock) as debug_mock:
            with mock.patch("lyngdorf.api.LyngdorfProtocol.write", new_callable=mock.Mock) as write_mock:
                ...
                after_list=list(map(lambda call:call.args[0].replace('\r',''), write_mock.call_args_list[before_length:]))
